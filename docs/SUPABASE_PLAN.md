...
- `notify-manager` sends authenticated shift-complete email through Resend.
- `end-of-day` runs from Supabase Cron and sends timezone-aware cutoff summaries.
- `notification_events` is the service-role-written audit/idempotency boundary; browser users have read-only manager-scoped access.
