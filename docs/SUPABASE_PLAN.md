...
New Auth-user creation and initial organisation membership provisioning are implemented through the authenticated, platform-admin-only `create-user` Edge Function. Existing profiles can receive additional organisation memberships, role changes, or organisation-access removal through the authenticated, platform-admin-only `manage-user-access` Edge Function and service-role-only `admin_manage_user_organisation_access(...)` RPC. If provisioning fails, `create-user` calls the service-role-only `cleanup_failed_created_user_profile(...)` RPC before attempting Auth deletion. Venue access remains a separate Team action. Ordinary managers cannot create Auth users or modify organisation memberships.
