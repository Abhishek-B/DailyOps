...
### Photo evidence pre-merge verification — 2026-09-23

The user requested review/regression verification and a local merge from `feature/task-photo-evidence` to `development`. The checks below supersede the deployment/uncommitted status recorded in the historical phase notes. No production data, files, recipients, secrets, migrations or schedules were changed during verification; no Git push or GitHub Pages publication was requested.

- **Local checks:** all 69 Node browser/database checks and 20 Deno photo/notification tests passed again. The four affected Edge Functions and both setup scripts type-check. Inline frontend JavaScript parses, all 75 literal `data-act` controls have dispatcher cases, and `git diff --check` passes. Existing venue administration/timing, employee day initialisation, shared-shift attribution/submission, stale-revision protection, template copy, roster search, notification outcomes, responsive layouts and demo coverage still pass. No regression was found in those tested paths; this is not proof of every production workflow.
- **Deployed notification code:** CLI registry reports `notify-manager` ACTIVE v6 and `end-of-day` ACTIVE v4. Downloaded entrypoints and their shared notification/Supabase helpers are byte-identical to the reviewed local files. `DAILYOPS_APP_URL` matches `https://abhishek-b.github.io/DailyOps/` by SHA-256 comparison without printing its hosted value. Hosted cron secret, local cron secret and Vault secret match; Vault project URL matches too. Both notification functions return 200 to OPTIONS and 401 to unauthenticated POST.
- **Photo services/schema:** upload and cleanup are ACTIVE v2 and reject unauthenticated POST with 401. The bucket is private, JPEG-only, and limited to 5 MiB. Database catalog checks confirm photo flags, organisation retention and migration-028 retry columns, evidence-table RLS and restrictive Storage read/upload/no-overwrite/no-delete boundaries. Authenticated users can submit/select exemptions but cannot call finalisation or cleanup; retention UPDATE privilege exists and organisation RLS remains authoritative. The service-role REST probe for organisations was denied by its existing table grant; catalog inspection confirmed the retention column without widening permissions.
- **Schedule:** the cleanup job is active at `*/15 * * * *` and references the expected endpoint/Vault secret. The three latest cron runs succeeded; matching existing cleanup HTTP results at 06:45, 07:00 and 07:15 UTC on 2026-09-23 were 200, not timed out, with `ok: true`. This verifies scheduled invocation, not physical deletion of an actual photo.
- **Frontend publication is pending:** the live GitHub Pages page returns 200 but does not yet contain photo controls or the evidence-link handler, and does not match this checkout. Until it is published, new Telegram evidence links reach the older app. Test the merged page locally or publish the intended build before testing hosted links. Keep operational photo requirements off until acceptance testing passes.
- **Not exercised live:** authenticated uploads/downloads, real camera/HEIC behaviour, actual photo deletion, Telegram delivery, cross-user sessions and concurrent submit/remove/reset/finalise races. No operational Telegram messages or synthetic production data were created. Use the isolated test-venue checklist before production enablement.

The focused review found no confirmed merge-blocking security/correctness issue. Evidence metadata is paginated but all pages are accumulated, and Today polling adds three evidence reads; checklist-key indexing/query-volume profiling is a low-priority scalability follow-up, not a measured regression. `.env` and generated decoder assets remain ignored; matching local secret values were absent from all 75 tracked/non-ignored candidate files.

### Feature branch: task photo evidence, phase 4 — 2026-09-23

Implemented locally on `feature/task-photo-evidence`, preserving the uncommitted phase-2/3 work. No commits, pushes, publication, secrets changes or deployments were performed. No new migration or Cron SQL is required. Before rollout, publish the updated static page, configure `DAILYOPS_APP_URL` as its full HTTPS URL, and redeploy `notify-manager` and `end-of-day` sequentially. Missing/invalid app URL configuration deliberately fails affected notifications rather than sending broken links. Upload/cleanup services need no phase-4 redeployment.

Submission notifications use immutable per-revision photo/exemption counts; reopen notifications use current ready/unexpired counts. EOD includes evidence summaries/links for each existing shift. Counts come from the database, never browser input, and evidence-read failures are surfaced rather than called zero. Telegram receives no photos or private Storage URLs, and link previews are disabled. Truncation preserves evidence links. Existing recipient preferences, per-recipient revision keys and retry/delivery auditing are unchanged.

The static hash link survives sign-in and opens a read-only venue/date/shift evidence list; submitted links pin the exact revision even after reset or beyond normal History pagination. Venue access is required and every read uses the signed-in RLS client. Invalid/unavailable links do not fall back to another shift. Expired-photo audit remains visible, photos download only on demand, and sign-out/access loss/context changes discard private previews and late responses. No framework or frontend build was introduced.

Local checks passed: 69 Node browser/database tests and 20 Deno photo/notification tests. They cover notification counts, pagination, long-message link retention, access denial, failed queries, preferences and retries; browser tests cover sign-in resume, old/reset revisions, cross-organisation selection, bad links, read-only views and access/context races. Hosted smoke testing is still pending the final deployment and user confirmation. The [root README phase-4 rollout](../README.md#phase-4-rollout-not-performed) and [photo deployment guide](../supabase/functions/upload-task-evidence/README.md#verification-before-production-enablement) describe the remaining steps. Keep production requirements off until that verification passes.

### Feature branch: task photo evidence, phase 3 — 2026-09-23

Implemented locally on `feature/task-photo-evidence`, preserving the existing uncommitted phase-2 work. No commits, pushes, frontend publication, new migrations or Edge Function redeployments were made. Phase 3 uses migrations 027/028 and the already-deployed phase-2 functions. The static single-file architecture and demo behaviour are unchanged.

The frontend now includes routine/one-off photo-required switches, requirement-preserving template copying, organisation retention settings, camera/gallery preparation (JPEG, 1600px maximum side), sequential uploads up to three live photos/reservations, same-ID/bytes retry and pending-upload discard. Required individual/bulk Done actions are gated; server guards remain authoritative. Uploading does not automatically complete a task. Photos remain private and are fetched only on request, with in-memory previews cleaned up on expiry, close, context change and access loss.

Managers can approve reasoned exemptions or revoke them, and submission explicitly acknowledges exactly the exemptions used instead of photos. Submitted/historical evidence is read-only. Evidence audit shows earlier revisions and reset/deleted-task submission records without losing expired-photo metadata. Evidence reads paginate; Today retains its existing 15-second refresh, and the panel supports explicit refresh. No image bytes, service credentials or signed URLs are persisted in frontend storage. Camera capture, HEIC support and browser memory behaviour still need real-device checks during the hosted smoke test.

Local verification passed: 61 Node browser/database tests and 11 Deno photo-function tests, inline JavaScript parsing and `git diff --check`. Desktop/phone screenshots were inspected, including the evidence panel, task actions and organisation settings.

The user reports the cleanup schedule SQL has been run. Scheduled HTTP success has not been independently checked. At the user's request, hosted smoke testing is deferred until all implementation phases are complete; local regression testing continues per phase. Keep production photo requirements off until the final end-to-end verification. Phase 4 was subsequently authorised and implemented locally as recorded above.

### Feature branch: task photo evidence, phase 2 — 2026-09-22

Implemented on the existing `feature/task-photo-evidence` branch, following the user's committed phase-1 changes. On 2026-09-23, read-only live checks confirmed the migration-028 cleanup columns and the private `task-evidence` bucket configured for JPEG only with a 5 MiB limit. No migration or bucket changes were made during deployment.

Phase 2 adds private-bucket provisioning, authenticated image validation/upload and scheduled physical deletion through the Storage API. The validator decodes JPEG/PNG/WebP, rejects oversized/animated/unsupported inputs, auto-orients, strips metadata and saves a resized JPEG without overwriting existing objects. The caller's JWT enforces Storage upload permissions; only the trusted function finalises metadata. Cleanup retains audit records, records failures/backoff and reconciles late-arriving uploads. The upload function's pinned WASM decoder is prepared as an ignored backend asset to exclude the unused second binary; static GitHub Pages hosting still requires no frontend build.

Deployment-guide step 3 completed on 2026-09-23: prepared the pinned decoder, bundled using Docker, and deployed `upload-task-evidence` (6.0 MB) and `cleanup-task-evidence` (762 kB), both active version 1. Cleanup required a sequential retry after its initial successful CLI response did not produce a registered endpoint; final registry and HTTP checks confirmed both functions. Existing function versions and hosted secrets were unchanged, and the cron-secret digest matched `.env`. Unauthenticated POST requests returned the expected 401 from each function; upload OPTIONS returned 200. All 11 Deno phase-2 tests and both function type checks passed again.

The user subsequently reported running [deployment-guide step 4](../supabase/functions/upload-task-evidence/README.md#4-add-the-separate-cleanup-schedule) to create the cleanup schedule. No cleanup was manually invoked during deployment, and no photos or test users were created or removed. Actual hosted image processing, physical deletion and multi-session locking remain unverified; the guide's isolated authenticated smoke test is still required after all phases. Local Deno tests use real image decoding and mocked HTTP; PGlite verifies migrations/RLS; browser tests remain isolated from production.

**Keep photo requirements off until the final hosted smoke test.** The phase-3 UI implementation is recorded above. Photo counts and sign-in-required notification links are phase 4. No frontend or notification changes were included in phase 2 itself.
...
All existing requirements default off. Do not enable them in production before the upload/UI phases are ready. The phase-2 implementation and deployment requirements are recorded above. Phase 3 adds controls and viewing; phase 4 adds photo-count notifications and authenticated links. Static GitHub Pages hosting and the existing demo frontend remain unchanged. This phase-1 section and the sections below are historical notes, not an independent check of live migration or service state.
...
Migration `007_scope_team_and_roster_writes.sql` keeps manager-created venue memberships and roster assignments inside the target venue's organisation, requires manager-rostered employees to be active, and preserves the existing authenticated self-cover insert path. Its original explicit venue-membership prerequisite for manager roster writes is replaced by migration 023.
...
Migration `022_team_access_editor.sql` adds the service-role-only `admin_apply_user_access(...)` RPC. It validates a requested final platform/org/venue access state and applies membership, employee venue access, current/future cleanup when an account is deactivated or an organisation is removed, notification-recipient cleanup, active state, and platform role in one transaction. It reuses migration 021's caller context and protected-account guards; it does not add browser execute access.

Migration `023_relax_roster_venue_eligibility.sql` updates manager roster INSERT/UPDATE policies so an active employee membership in the venue's organisation is sufficient; permanent `venue_members` access is not required. The authenticated self-cover path still requires the caller's normal venue access. It also replaces the batch Team access RPC so removing permanent venue access does not delete future roster or cover rows, while deactivation and organisation removal retain their cleanup behavior.
...
- Employee deactivation and organisation removal clear relevant future roster assignments while retaining historical records. Removing permanent venue access alone does not clear future roster assignments.
...
- Platform-admin Team access editor with a local draft/save modal, discard confirmation, per-organisation role and employee venue controls, platform User/Admin editing, atomic final-state save, and protected-master/self-demotion safeguards.
- Team People/Roster separation: Team is a compact People/access directory and Roster is a dedicated manager page with Week and Day planning views over the shared `roster_assignments` rows.
- Roster planner redesign: employee-by-day assignment board, sticky employee column, venue/people filters, coverage gaps for supported operations, side-drawer editing, read-only past dates, and idempotent Copy previous week behavior using existing RLS-scoped writes.
- Roster eligibility: managers may schedule any active employee in the target venue's organisation without granting permanent venue access; missing permanent access, same-day multi-venue/operation assignments, and timed overlaps are advisory warnings, while exact duplicate assignment keys remain blocked.
