...
Migration `022_team_access_editor.sql` adds the service-role-only `admin_apply_user_access(...)` RPC. It validates a requested final platform/org/venue access state and applies membership, employee venue access, current/future roster and cover cleanup, notification-recipient cleanup, active state, and platform role in one transaction. It reuses migration 021's caller context and protected-account guards; it does not add browser execute access.
...
- Team People/Roster separation: compact People access summaries with the existing weekly roster planner retained under a dedicated Roster view.
