...
Roster assignment
- records a user, venue, date and operation assignment using the current legacy `shift_type` values `open` and `close`
- managers create/remove assignments through RLS-scoped writes; an active employee membership in the venue's organisation is sufficient for manager scheduling, so permanent `venue_members` access is not required
- permanent venue access, same-day multi-venue/operation assignments, and usable timed overlaps are surfaced as advisory planner warnings; only the exact venue/user/date/operation duplicate is rejected
- the dedicated Roster page presents the same rows in Week and Day planning views across all managed venues for a selected seven-day window
- the frontend iterates an operation metadata map so future operation types can be added without rebuilding the planner
...
- `manage-user-access` requires an authenticated bearer token, verifies an active platform-admin profile, and calls a service-role-only RPC: the legacy `admin_manage_user_organisation_access(...)` action for compatibility or the atomic final-state `admin_apply_user_access(...)` action for the Team access editor
...
The browser key is publishable. RLS policies use the signed-in Auth UUID, organisation memberships, venue memberships, active profiles, and the platform-admin helper to decide access. Ordinary managers inherit access to all venues in their organisation; active platform admins inherit management access across organisations; employees require an active venue membership to access venue operations. Manager-created roster assignments additionally allow any active employee membership in the target venue's organisation, without silently creating permanent venue access. Managers can administer profiles, venue memberships, and rosters only within managed organisations/venues; they cannot write `organisation_members`. Migration 020 restricts venue membership inserts/updates/deletes to employee memberships in the target venue organisation and protects browser platform-role changes. Migration 023 replaces only the roster assignment manager INSERT/UPDATE policies and preserves the employee self-cover path. Migration 020 also exposes only the service-role-only organisation-access RPC, which is called by the authenticated platform-admin `manage-user-access` Edge Function. The browser does not create Auth users directly. `create-user` and `manage-user-access` independently verify active platform-admin callers. Database triggers prevent employees from changing task definition fields, attributing a completion to another user, selecting a notification revision, changing protected venue timing settings, or changing `platform_role` through the browser.

Migration 021 adds a UUID-backed `protected_accounts` configuration for the master administrator. The migration resolves the existing bootstrap Auth user once, validates its profile is active and `platform_role = 'admin'`, and stores only the UUID and protection kind. Database triggers prevent other callers or privileged application paths from deactivating/demoting/deleting the protected profile or mutating its organisation/venue membership rows. The caller-aware organisation-access RPC and Edge Function reject attempts to target it from another account. Migration 022's batch RPC sets the same verified caller context before its writes, so the same guards apply to draft saves. The frontend reads only a safe protected flag through `is_protected_master_admin(uuid)`; it never contains the master UUID or email as a security rule.

The platform-admin Team People view keeps access edits in a local draft. A dirty draft requires an explicit save or discard, and the browser submits the desired final state once through `manage-user-access`; the database RPC applies it atomically. Platform role is an independent axis from each organisation role, self-demotion is rejected, and the weekly roster planner is separated into a Roster view to keep the People directory readable.
