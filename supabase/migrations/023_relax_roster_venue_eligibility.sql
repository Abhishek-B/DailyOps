-- DailyOps roster eligibility is organisation-scoped, not permanent-venue-access-scoped.
-- Migrations 001-022 remain immutable.
      ...
      or (
        ...
        and exists (
          ...
          join public.profiles p
            on p.id = roster_assignments.user_id
...
-- The Team access editor synchronises permanent venue_members rows. That must
-- not erase future roster assignments: managers may schedule active employees
-- across venues as an operational decision without granting permanent access.
...
begin
  ...
  for v_org in select value from jsonb_array_elements(p_organisations) loop
    ...
    if v_role = 'employee'::public.app_role then
      ...
      -- Permanent venue access changes do not remove roster or cover rows.
      -- Deactivation and organisation removal below retain their stronger
      -- cleanup semantics.
